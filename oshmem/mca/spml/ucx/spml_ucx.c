...
int mca_spml_ucx_ctx_create(long options, shmem_ctx_t *ctx)
{
    ...
    for (i = 0; i < MCA_MEMHEAP_SEG_COUNT; i++) {
        spml_ucx_mkey_t *ucx_mkey;
        ucx_mkey = &mca_spml_ucx_ctx_default.ucp_peers[my_pe].mkeys[i].key;
        ...
        err = ucp_ep_rkey_unpack(ctx_item->ctx.ucp_peers[oshmem_group_self->my_pe].ucp_conn,
                                 mkeys[0].u.data,
                                 &ucx_mkey->rkey);
        if (UCS_OK != err) {
            SPML_UCX_ERROR("failed to unpack rkey");
        ...
        mkeys[0].len     = len;
        mkeys[0].va_base = memheap_map->mem_segs[i].super.va_base;

        mca_spml_ucx_cache_mkey(&ctx_item->ctx, &mkeys[0], i, my_pe);
    }
