...
static inline
ucc_status_t mca_coll_ucc_reduce_scatter_init(const void *sbuf, void *rbuf,
                                               const int *rcounts,
                                               ...
                                               mca_coll_ucc_req_t *coll_req)
{
    ...
    if (MPI_IN_PLACE == sbuf) {
        /* TODO: UCC defines inplace differently:
           data in rbuf of rank R is shifted by sum(rcounts[0..R-1]) */
    ...
    /* Calculate total count for source buffer */
                                ...
                                mca_coll_base_module_t *module)
{
    ...
    COLL_UCC_CHECK(mca_coll_ucc_reduce_scatter_init(sbuf, rbuf, rcounts,
                                                     dtype, op, ucc_module,
                                                     &req, NULL));
...
fallback:
    ...
    return ucc_module->previous_reduce_scatter(sbuf, rbuf, rcounts, dtype,
                                               op, comm,
...
fallback:
    ...
    return ucc_module->previous_ireduce_scatter(sbuf, rbuf, rcounts, dtype,
                                                op, comm, request,
